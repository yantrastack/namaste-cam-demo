...
import {
  mergeOrderHistoryWithArchives,
  ORDER_ARCHIVES_EVENT,
  readOrderArchivesMap,
} from "@/lib/order-session-archives";
...
function historyStatusBadge(o: RestaurantOrderRecord) {
  if (o.status === "cancelled") return <Badge tone="error">Cancelled</Badge>;
  return <Badge tone="success">Completed</Badge>;
}

function closedTimeDisplay(o: RestaurantOrderRecord) {
  if (o.status === "cancelled") return o.cancellation?.cancelledAtLabel ?? "—";
  return o.completedAtLabel ?? "—";
}
...
export function OrderHistoryClient({ orders }: Props) {
  ...
  const mergedOrders = useMemo(
    () => mergeOrderHistoryWithArchives(orders, archiveMap),
    [orders, archiveMap],
  );
  ...
  const postcodeOptions = useMemo(() => {
    const set = new Set(mergedOrders.map((o) => o.postcode.trim()));
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableHeaderCell>Return payment</TableHeaderCell>
          ...
          <TableBody>
            {rows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={12} className="py-12 text-center text-sm font-medium text-secondary">
              ...
              pageRows.map((o) => {
                ...
                const paymentLabel = formatCheckoutPaymentSummary(o);
                const closedLabel = closedTimeDisplay(o);
                return (
                  <TableRow
                    key={o.id}
                    className="cursor-pointer hover:bg-surface-container-low/60"
                    tabIndex={0}
                    title={`View order ${o.code}`}
                  ...
                  >
                    ...
                    <TableCell>{historyStatusBadge(o)}</TableCell>
                    <TableCell>
                      <span className="text-sm font-bold text-on-surface">{paymentLabel}</span>
                    </TableCell>
                    <TableCell>
                      {o.status === "cancelled" ? (
                        o.cancellation?.returnPayment ? (
                          <span className="text-sm font-extrabold text-primary">Yes</span>
                        ) : (
                          <span className="text-sm font-medium text-secondary">No</span>
                        )
                      ) : (
                    ...
                    <TableCell className="text-sm font-medium text-secondary">{closedLabel}</TableCell>
                    <TableCell>
